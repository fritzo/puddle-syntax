...
fragments.church = fragment([
    ...
    'ASSERT', 'DEFINE', 'CURSOR', 'PAREN',
]);

var isInFragment = function (fragment) {
    var string = pattern.variable('string', _.isString);
    var array = pattern.variable('array');

    var t = pattern.match(
        string, function (match) {
            return _.has(fragment, match.string);
        },
        VAR(string), function (match) {
            return _.has(fragment, 'VAR');
        },
        array, function (match) {
            var head = match.array[0];
            var tail = match.array.slice(1);
            return _.has(fragment, head) && _.all(tail, t);
        }
    );
...
var isCurry = isInFragment(fragments.curry);
var isChurch = isInFragment(fragments.church);

var isStatement = (function () {
    var statementTokens = {ASSERT: true, DEFINE: true};
    return function (term) {
        return _.isArray(term) && statementTokens[term[0]];
    };
})();

var isExpression = function (term) {
    return !isStatement(term);
};

//----------------------------------------------------------------------------
// Conversion : appTree <-> curryTerm
...
var decompile = (function () {
    ...
    var decompile = function (curryTerm) {
        return decompileStack(toStack(curryTerm));
    };

    return function (curryTerm) {
        //assert(isCurry(curryTerm), curryTerm);  // FIXME
        fresh.reset();
        var churchTerm = decompile(curryTerm);
        churchTerm = simplify(churchTerm);
        return churchTerm;
    };
})();
...
var compile = (function () {
    ...
    return function (churchTerm) {
        //assert(isChurch(churchTerm), churchTerm); // FIXME
        return t(churchTerm);
    };
})();
...
module.exports = {
    ...
    load: function (string) {
        var curryTerm = parse(string);
        var churchTerm = decompile(curryTerm);
        return churchTerm;
    },
    dump: function (churchTerm) {
